...
public class ZooKeeperServer implements SessionExpirer, ServerStats.Provider {
    ...
    public ZooKeeperServer(FileTxnSnapLog txnLogFactory, int tickTime,
        ...
        serverStats = new ServerStats(this);  // todo 创建ServerStats对象
    ...
    // todo 初始化ZKDatabase数据结构，该数据结构用于保存ZNode节点
    public void startdata()
    throws IOException, InterruptedException {
        //check to see if zkDb is not null
        if (zkDb == null) {
            // todo 初始化ZKDataBase，这里会添加例如'/zookeeper'等原始节点
            zkDb = new ZKDatabase(this.txnLogFactory);
        }
        // todo 如果磁盘上已存有数据，则加载该数据（恢复本地数据）
        if (!zkDb.isInitialized()) {
            loadData();
        }
    }
    ...
    public synchronized void startup() {
        ...
        // todo 注册jmx
        //  JMX全程Java Management Extensions，是管理Java的拓展机制，这种机制可以很方便的管理、监控运行中的Java程序。
        registerJMX();
        // todo 成功注册zk服务器实例，状态变更为RUNNING
        //  四种状态： 启动过程中-INITIAL、启动完成-RUNNING、内部错误-ERROR、停机-SHUTDOWN
        //  当处于内部错误-ERROR或停机-SHUTDOWN，setState内部会执行`shutdownLatch.countDown();`，从而执行shutdown操作
        setState(State.RUNNING);
        notifyAll();
    }

    // todo
    //  PrepRequestProcessor：第一个执行，为请求添加事务请求头，执行一些检查操作
    //  SyncRequestProcessor：第二个执行，将请求记录到事务日志中
    //  FinalRequestProcessor：最后执行，负责执行具体的请求任务
